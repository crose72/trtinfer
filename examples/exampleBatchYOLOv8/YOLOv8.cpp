...
std::vector<std::vector<cv::cuda::GpuMat>> YOLOv8::preprocess(const std::vector<cv::cuda::GpuMat> &gpuImgs)
{
    ...
    for (const auto &img : gpuImgs)
    {
        // Convert the image from BGR to RGB
        cv::cuda::GpuMat rgbMat;
        cv::cuda::cvtColor(img, rgbMat, cv::COLOR_BGR2RGB);
        ...
        if (resized.rows != inputDims[0].d[1] || resized.cols != inputDims[0].d[2])
        {
            ...
            input.push_back(std::move(resized));

            // Store original dimensions and aspect scale factor for each image
            mInputImgHeights.push_back(rgbMat.rows);
            mInputImgWidths.push_back(rgbMat.cols);

            // Aspect scale for bounding box rescaling
            float aspectScale = 1.f / std::min(
                                          inputDims[0].d[2] / static_cast<float>(rgbMat.cols),
                                          inputDims[0].d[1] / static_cast<float>(rgbMat.rows));
            mAspectScaleFactors.push_back(aspectScale);
        }
    }

    // Convert to format expected by our inference engine
    // The reason for the strange format is because it supports models with multiple inputs as well as batching
    // This format: [input][batch][GpuMat], so 1 input, batch of images
    std::vector<std::vector<cv::cuda::GpuMat>> inputs{input};
    return inputs;
}
...
std::vector<std::vector<Object>> YOLOv8::detectObjects(const std::vector<cv::cuda::GpuMat> &batchImgs)
{
    ...
    auto inputs = preprocess(batchImgs);

    // Run inference
    std::vector<std::vector<std::vector<float>>> featureVectors;
    bool succ = mEngine->runInference(inputs, featureVectors);
    if (!succ)
    {
        throw std::runtime_error("Error: Unable to run batch inference.");
    }

    std::vector<std::vector<Object>> batchResults;

    // Loop through each image in the batch
    for (size_t i = 0; i < batchImgs.size(); ++i)
    {
        // Prepare output vectors for this image
        std::vector<std::vector<float>> featureVector;
        // Assuming transformOutput will extract the i-th output correctly for batch
        transformOutput(featureVectors, featureVector); // You may need to update transformOutput to take batchIdx

        // Now postprocess for this image (use per-image geometry!)
        mInputImgHeight = mInputImgHeights[i];
        mInputImgWidth = mInputImgWidths[i];
        mAspectScaleFactor = mAspectScaleFactors[i];
        std::vector<Object> objs = postprocessDetect(featureVector[i]);
        batchResults.push_back(objs);
    }
    return batchResults;
}
